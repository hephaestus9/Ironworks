...
Ext.define("Bleext.modules.catalogs.roles.controller.Roles",{
	...
	saveRole	: function(){
		...
		Bleext.Ajax.request({
			url		: Bleext.desktop.Constants.SAVE_ROLES,
			params	: {roles : Ext.encode(data)},
			el		: this.win.el,
			success	: function(info,options){
				Ext.Msg.alert("Message",info.message);
				store.load();
			},
			failure	: function(info){
				Bleext.log(info);
				Ext.Msg.alert("Error","No roles were saved!");
			}
		});
	},
			...
			Ext.Msg.confirm(
				...
				function(btn){
					if(btn === "yes"){
						...
						Bleext.Ajax.request({
							url		: Bleext.desktop.Constants.REMOVE_ROLE,
							params	: {roles:Ext.encode(data)},
