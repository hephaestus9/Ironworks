...
def recently_added_db_add(server, media_type, recently_added):
    ...
    if RecentlyAdded.query.filter(RecentlyAdded.name == name).first() is None:
...
def cache_recent_image(label, type, id, image):
    ...
    if not os.path.exists(file_path):
        image_path = ironworks_path() + xbmc_image(image, label)
