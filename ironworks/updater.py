...
DATA_DIR = serverTools.getDataDir()
USE_GIT = serverTools.getUseGit()
CURRENT_COMMIT = serverTools.getCurrentCommit()
LATEST_COMMIT = serverTools.getLatestCommit()
COMMITS_BEHIND = serverTools.getCommitsBehind()
FIRST_RUN = serverTools.getFirstRun()

# define master repo as user and branch in github repo
user = 'Sir-Henry-Curtis'
branch = 'master'
...
def latestCommit():
    ...
    url = 'https://api.github.com/repos/%s/ironworks/commits/%s' % (user, branch)
...
def commitsBehind():
    ...
    url = 'https://api.github.com/repos/%s/ironworks/compare/%s...%s' % (user, CURRENT_COMMIT, LATEST_COMMIT)
    result = urllib2.urlopen(url).read()
    git = json.JSONDecoder().decode(result)
...
def checkGithub():
    ...
    try:
        ...
        serverTools.setLatestCommit(LATEST_COMMIT)
        if FIRST_RUN:
    ...
    if CURRENT_COMMIT:
        ...
        if COMMITS_BEHIND >= 1:
            logger.log('UPDATER :: Update available, you are %i commits behind' % COMMITS_BEHIND, 'INFO')
            COMMITS_COMPARE_URL = 'https://github.com/%s/ironworks/compare/%s...%s' % (user, CURRENT_COMMIT, LATEST_COMMIT)
            serverTools.setCommitsCompareURL(COMMITS_COMPARE_URL)
...
def RemoveUpdateFiles():
    ...
    tar_file = joinRundir('ironworks.tar.gz')
    update_folder = joinRundir('ironworks-update')
...
def Update():
    ...
    try:
        ...
        url = urllib2.urlopen('https://github.com/%s/ironworks/tarball/%s' % (user, branch))
        f = open(tar_file, 'wb')
        f.write(url.read())
        f.close()
    except:
    ...
    root_src_dir = os.path.join(update_folder, '%s-ironworks-%s' % (user, LATEST_COMMIT[:7]))
    ...
    CURRENT_COMMIT = LATEST_COMMIT
    COMMITS_BEHIND = 0
    serverTools.setCurrentCommit(CURRENT_COMMIT)
    serverTools.setCommitsBehind(COMMITS_BEHIND)
